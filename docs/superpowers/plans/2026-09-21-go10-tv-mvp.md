...
Run: `npm run dev`
Open the printed URL. Expected: the page reads `858 títulos` (774 movies + 84 shows = 907 videos with 133 seasons collapsed) on the dark canvas. Confirm the exact number matches `buildTitles` output rather than assuming — record it, since Task 11 re-checks it.
