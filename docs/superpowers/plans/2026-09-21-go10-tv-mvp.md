...
def test_build_rows_produces_full_catalog():
    ...
    assert len({r["series_id"] for r in rows if r["series_id"]}) == 85
...
Produce `data/genres.csv` covering all 907 videos, then regenerate the catalog with genres joined in.

**Files:**
- Create: `data/genres.csv`
- Create: `scripts/seed_genres.py`
- Create: `tests/test_genres.py`
- Modify: `public/data/catalog.csv` (regenerated)

**Interfaces:**
- Consumes: `extract_cards`, `COLUMNS` from `scripts/parse_catalog.py`; `parse_title` from `scripts/title_parser.py`.
- Produces: `data/genres.csv` with columns `video_id,title,genre,genre_secondary`.
...
def load_genres():
    with open(os.path.join(ROOT, "data", "genres.csv"), encoding="utf-8", newline="") as handle:
        return list(csv.DictReader(handle))


def test_every_video_has_a_genre_row():
    html_text = open(os.path.join(ROOT, "catalogo-solo-videos.html"), encoding="utf-8").read()
...
def test_genres_use_the_closed_vocabulary():
    for row in load_genres():
        ...
        assert row["genre"] != row["genre_secondary"] or row["genre_secondary"] == "", row


def test_catalog_csv_has_genres_joined():
    with open(os.path.join(ROOT, "public", "data", "catalog.csv"), encoding="utf-8", newline="") as handle:
        rows = list(csv.DictReader(handle))
    assert all(r["genre"] in VOCABULARY for r in rows)
```
...
- [ ] **Step 3: Write the heuristic seeder**

`scripts/seed_genres.py`:

```python
"""Seed data/genres.csv with heuristic guesses for every video.

Genre is INFERRED, not scraped. This seeder produces a first pass from studio
and language signals; the remaining rows are marked UNCLASSIFIED for manual
review in the next step. Re-running never overwrites an existing row, so
...
ANIMATION_STUDIOS = {"Disney", "Pixar", "Cartoon N.", "DreamWorks", "Nickelodeon", "Adult Swim"}
SUPERHERO_STUDIOS = {"DC", "Marvel"}


def guess(parsed):
    """Return (genre, genre_secondary) from signals present in the title."""
    studio = parsed["studio"]
    if studio in SUPERHERO_STUDIOS:
        return "Superhéroes", "Acción"
    if studio in ANIMATION_STUDIOS:
        return "Animación", "Infantil"
    if parsed["subtitled"]:
        return "Anime", ""
    year = parsed["year"]
...
def main():
    ...
    print(f"wrote {len(rows)} rows, {unclassified} still UNCLASSIFIED")
...
Expected: `wrote 907 rows, N still UNCLASSIFIED`

- [ ] **Step 5: Classify the remaining titles**
...
python3 -c "
import csv
rows = [r for r in csv.DictReader(open('data/genres.csv', encoding='utf-8')) if r['genre'] == 'UNCLASSIFIED']
for r in rows: print(r['video_id'], r['title'], sep='\t')
"
```

Work through the list in batches of roughly 50, editing `data/genres.csv` in place. For each title, assign a `genre` from the closed vocabulary and optionally a distinct `genre_secondary`. Also correct any heuristic guess that is plainly wrong — notably, the `subtitled -> Anime` rule catches live-action foreign films too, and the `year < 1970 -> Clásicos` rule catches early animation.

When a title is genuinely unrecognisable, assign the closest defensible genre rather than leaving it blank — `test_genres_use_the_closed_vocabulary` fails on `UNCLASSIFIED`, and an empty primary genre would break row building.

- [ ] **Step 6: Regenerate the catalog with genres joined**

Run: `python3 scripts/parse_catalog.py`
Expected: coverage report now shows `genre 907/907`

- [ ] **Step 7: Run tests to verify they pass**

Run: `python3 -m pytest tests/ -v`
Expected: PASS — all green, including the three genre tests

- [ ] **Step 8: Commit**

```bash
git add data/genres.csv scripts/seed_genres.py tests/test_genres.py public/data/catalog.csv
    ...
    <div className="go-player">
      ...
      {failed ? (
        <div className="go-player_fallback">
          ...
          <a className="go-play is-focused" href={row.video_url} target="_blank" rel="noreferrer">
            Abrir en ok.ru
          </a>
        </div>
      ) : (
...
.go-player_fallback a { text-decoration: none; color: var(--go-text); }
```
